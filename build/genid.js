...
class GenidService {
    constructor(options) {
        ...
        this.genId = async () => {
            ...
            this.lastTime = time;
            let bSeq = this.seq.toString(2), bMachineId = this.machineId.toString(2), bProcessId = this.processId.toString(2), bTime = (time - this.timeOffset).toString(2);
            //create sequence binary bit
            while (bSeq.length < SEQUENCE_LENGTH)
                bSeq = "0" + bSeq;
            while (bMachineId.length < MACHINE_LENGTH)
                bMachineId = "0" + bMachineId;
            while (bProcessId.length < PROCESS_LENGTH)
                bProcessId = "0" + bProcessId;
            while (bTime.length < TIMESTAMP_LENGTH)
                bTime = "0" + bTime;
...
exports.default = GenidService;
GenidService.ID_LENGTH = ID_LENGTH;
